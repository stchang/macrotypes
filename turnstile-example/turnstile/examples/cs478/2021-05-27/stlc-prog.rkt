...
(check-type (inst (Λ X (λ [x : X] x)) {Int}) : (→ Int Int))

(check-type ((inst (Λ X (λ [x : X] x)) {Int}) 25) : Int -> 25)

(define id (Λ X (λ [x : X] x)))

(check-type (id 25) : Int -> 25)
(check-type (id #f) : Bool -> #f)

(check-type (∀ (A) (∀ (B) (→ A B))) :: #%type)

(check-type (∀ (A) (∀ (B) (→ (→ A B) A B))) :: #%type)

(check-type
 (Λ A (Λ B (λ ([f : (→ A B)] [a : A]) (f a))))
 : (∀ (A) (∀ (B) (→ (→ A B) A B))))
...
(check-type (unpack [Y y] (pack (Int 0) as (∃ (X) X))
                    in
                    ;; this fn app checks that y has type Y,
                    ;; but returns something with another type
                    ;; because Y cannot escape unpack body's scope
                    ((λ [z : Y] 1) y))
            : Int)

(typecheck-fail
 (unpack [Y y] (pack (Int 0) as (∃ (X) X))
         in
         ;; this fn app checks that y should not type check with Int
         (ascribe y as Int))
 #:with-msg "expected Int, given Y")
...
(define/unpack [Counter counter] counterADT)
(define/unpack [Counter2 counter2] counterADT)

(check-type (proj counter new) : Counter)

(typecheck-fail (ascribe (proj counter new) as Counter2)
                #:with-msg "expected Counter2, given Counter")

(define count1 (proj counter new))
(define inc (proj counter inc))
(define get (proj counter get))

(check-type count1 : Counter)

(check-type (inc count1) : Counter -> 2)
(check-type (get (inc count1)) : Int -> 2)

(check-type (succ (get (inc count1))) : Int -> 3)
(typecheck-fail (succ count1) #:with-msg "expected Int, given Counter")

(define counter-add3
  (λ [c : Counter] (inc (inc (inc c)))))

(check-type (counter-add3 count1) : Counter -> 4)
(check-type (get (counter-add3 count1)) : Int -> 4)
