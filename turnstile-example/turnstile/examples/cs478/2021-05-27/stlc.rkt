...
(define-typed-syntax typed-app
  [(_ e_fn e_arg ...) ≫
   [⊢ e_fn ≫ e_fn- ⇒ (~→ τ_in ... τ_out ~!)] ; dont backtrack when seeing →
...
#;  [(_ e_fn e_arg) ≫
   [⊢ e_fn ≫ e_fn- ⇒ (~∀ (X) (~→ τin τout))]
   [⊢ e_arg ≫ e_arg- ⇒ τarg]
   [($subst τarg X τin) τ= τarg]
   --------
   [⊢ (#%app- e_fn- e_arg-) ⇒ ($subst τarg X τout)]]
  [(_ e_fn e_arg) ≫
   [⊢ e_fn ≫ e_fn- ⇒ (~∀ (X) τf)]
   [⊢ e_arg ≫ e_arg- ⇒ τarg]
   --------
   [≻ (typed-app (inst e_fn- {τarg}) e_arg-)]])
...
#;  [(proj e l:id) ≫
  ...
  [(proj e l:id) ≫
   ...
   ----------------------
   [⊢ (let- ([res (assoc- 'l e-)])
        (unless- res (error- 'proj (format- "no field ~a" 'l)))
        (cdr- res))    ⇒ τ]]
  ;; tup proj ----------------------------------------
  [(proj e i:nat) ≫
   -----------
        ...
        (syntax-parse (list t1 t2)
          ...
          [((~Pair t1 t2) (~Pair t3 t4))
           (and (subtype? #'t1 #'t3)
                (subtype? #'t2 #'t4))]
          [((~× t1 ...) (~× t2 ...))
           (and (<= (stx-length #'(t2 ...)) (stx-length #'(t1 ...)))
                (for/and ([t1 (in-stx-list #'(t1 ...))]
                          [t2 (in-stx-list #'(t2 ...))])
                  (subtype? t1 t2)))]
          [((~Rec [x = τ1] ...) (~Rec [y = τ2] ...))
          ...
          [((~Ref τ1) (~Ref τ2))
           ...
           (case (current-ref-variance)
             [(invariant) (and (subtype? #'τ1 #'τ2) (subtype? #'τ2 #'τ1))]
             [(covariant) (subtype? #'τ1 #'τ2)]
             [(contravariant) (subtype? #'τ2 #'τ1)])]
          [(_ ~Top) #t]
          [(_ _) #f])))    
...
(define-typerule (Λ X:id e) ≫
  [[X ≫ X- :: #%type] ⊢ e ≫ e- ⇒ τ]
  ----------------
  [⊢ e- ⇒ (∀ (X-) τ)])

(define-typerule (inst e τ:type-ann) ≫
  [⊢ e ≫ e- ⇒ (~∀ (X) τ2)]
  ----------------
  [⊢ e- ⇒ ($subst τ.norm X τ2)])
...
(provide pack unpack ∃ define/unpack)

(define-binding-type ∃ #:bvs = 1)

(define-typerule (pack [τ:type e] (~datum as) ∃τ:type) ≫
  #:with (~∃ (X) τbody) #'∃τ.norm
  [⊢ e ≫ e- ⇐ ($subst τ.norm X τbody)]
  -------------------------------
  [⊢ e- ⇒ ∃τ.norm])

;; TODO: check that X does not escape unpack's scope
(define-typerule (unpack [X x] e1 (~datum in) e2) ≫
  [⊢ e1 ≫ e1- ⇒ (~∃ (Y) τbody)]
  [[X ≫ X- :: #%type] [x ≫ x- : ($subst X Y τbody)] ⊢ e2 ≫ e2- ⇒ τ2]
  ----------------
  [⊢ (let- ([x- e1-]) e2-) ⇒ τ2])

(define-typerule (define/unpack [X x] e1) ≫
  [⊢ e1 ≫ e1- ⇒ (~∃ (Y) τbody)]
;  [[X ≫ X- :: #%type] [x ≫ x- : ($subst X Y τbody)] ⊢ e2 ≫ e2- ⇒ τ2]
  ----------------
  [≻ (begin-
       (define-base-type X)
       (define-typed-variable x e1- ⇒ ($subst X Y τbody)))])
