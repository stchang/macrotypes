...
(define-typed-syntax test #:datum-literals (?)
  ...
  [(_ [τ0+:type ? (unop x-stx:id n-stx:nat)] e1 e2)
   ...
   #:with [x1 e1+ τ1] (parameterize ([eval-varassign? #t])
                        (infer/ctx+erase #'([x-stx : τ+]) #'e1))
   #:with [x2 e2+ τ2] (parameterize ([eval-varassign? #t])
                        (infer/ctx+erase #'([x-stx : τ-]) #'e2))
  ...
  [(_ [τ0+:type ? x-stx:id] e1 e2)
   ...
   #:with τ0- (∖ #'τ0 #'τ0+)
   #:with [x1 e1+ τ1] (parameterize ([eval-varassign? #t])
                        (infer/ctx+erase #'([x-stx : τ0+]) #'e1))
   #:with [x2 e2+ τ2] (parameterize ([eval-varassign? #t])
                        (infer/ctx+erase #'([x-stx : τ0-]) #'e2))
