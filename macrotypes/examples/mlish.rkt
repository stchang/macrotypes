  ...
  (define (solve Xs tyXs stx)
    (syntax-parse tyXs
      [(τ_inX ... τ_outX)
       ...
       #:with (~?∀ Vs expected-ty)
              ;; I think this type-eval is the same reason as dep.rkt lambda
              ;; (bc var-assigned are still eval'ed)
              ;; the teval is needed here but not in the turnstile version
              ;; bc the turnstile version tevals the expected type in the lam rule
         ...
         (for/fold ([exprs (make-list (length variance-vars) irrelevant)])
           ...
           (define/syntax-parse (~?∀ Xs* τ*)
             ...
             (with-variance-vars-okay
               ;; this type-eval is needed since τ may come from the user
               ;; (from define-type)
...
(define-typed-syntax for/list
  [(for/list ([x:id e]...) body)
   #:with ([e- (ty)] ...) (⇑s (e ...) as Sequence)
   #:with [(x- ...) body- ty_body] (infer/ctx+erase #'([x : ty] ...) #'body)
   (assign-type #'(for/list- ([x- e-] ...) body-) (mk-List- #'(ty_body)) #:eval? #f)])
...
(define-typed-syntax write-string
 ...
 [(write-string str out start end)
   ...
   #:with start- (⇑ start as Int)
   #:with end- (⇑ end as Int)
   (⊢/no-teval (begin- (write-string- str- out- start- end-) (void-)) : #,Unit+)])
...
(module+ test
  (begin-for-syntax
    (define (covariant-Xs?* t) (covariant-Xs? ((current-type-eval) t)))
