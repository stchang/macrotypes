  ...
  (define (make-constructor-transformer Xs τs Name-arg StructName-arg Name? mk-Name-)
    ...
    (lambda (stx)
      (syntax-parse/typecheck stx
        ...
        [:id ≫
         #:when (not (stx-null? #'(τ ...)))
         #:with tyout ((current-type-eval) #'(∀ (X ...) (ext-stlc:→ τ ... (Name X ...))))
         --------
         [⊢ StructName ⇒ ty-out]]
        [(C τs e_arg ...) ≫
...
(define-typed-syntax match2
  [(_ e (~datum with) . clauses) ≫
   ...
   ----
   [⊢ (match- e- [pat- (let- ([x- x] ...) e_body-)] ...) ⇒ #,(apply (current-join) (stx->list #'(ty_body ...)))]])
      ...
      (syntax-parse/typecheck #'clauses
       [([(~or (~and (~and xs [x ...]) (~parse rst (generate-temporary)))
        ...
        [⊢ (let- ([z e-])
           ...
           ⇒ #,(apply (current-join) (stx->list #'(ty_body ...)))]])]
     [else  ;; e is variant
      (syntax-parse/typecheck #'clauses
       [([Clause:id x:id ... 
        ...
        [⊢ (let- ([z e-])
             (cond- 
              [(and- (Cons? z) 
                     (let- ([x- (acc z)] ...) e_guard-))
               (let- ([x- (acc z)] ...) e_c-)] ...))
           ⇒ #,(apply (current-join) (stx->list #'(τ_ec ...)))]])])
    --------
    [≻ out]])
         ...
         #:with τ_out* (if (stx-null? #'(unsolved-X ...))
                           ...
                           (syntax-parse #'τ_out
                             [(~?∀ (Y ...) τ_out)
                              ...
                              (mk-∀- #'(unsolved-X ... Y ...) #'τ_out)]))
;                              #'(∀ (unsolved-X ... Y ...) τ_out)]))
          (assign-type #'((#%app- e_fn- op ...) e_arg- ...) #'τ_out* #:eval? #f)])])])]]
...
(define-typed-syntax cond
  ...
  [(cond [(~or (~and (~datum else) (~parse test #'(ext-stlc:#%datum . #t)))
   ...
   --------
   [⊢ (cond- [test- body-] ...) ⇒ #,(apply (current-join) #'(τ_body ...))]])
...
;       ⇒ (∀ Xs (ext-stlc:→ . ty-args))]] ; conc type, TCs dropped
       ⇒ #,(mk-∀- #'Xs (mk-→- #'ty-args))]] ; conc type, TCs dropped
   ;; base type --------------------------------------------------
