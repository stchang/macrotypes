...
(begin-for-syntax
  ...
  ;; - ASSUME: the needed indices are first `num-is` arguments in x+τss
  ;; - ASSUME: the recursive arg has type (TY . args) where TY is unexpanded
  (define (find-recur/i2 TY num-is x+τss)
    (stx-map
     (λ (x+τs)
       (define xs (stx-map stx-car x+τs))
       (stx-filtermap
        (syntax-parser
;          [_ #:do[(printf "find recur2: ~a\n" (stx->datum this-syntax))] #:when #f void]
          [(x (t:id . _)) ; unexpanded
           (and (free-id=? #'t TY) (cons #'x (stx-take xs num-is)))]
          ;; TODO: generalize these patterns with ~plain-app/c
;          [(x (_ t:id . _)) (and (free-id=? #'t TY) (cons #'x (stx-take xs num-is)))]
;          [(x (_ (_ t:id . _) . _)) (and (free-id=? #'t TY) (cons #'x (stx-take xs num-is)))]
          [_ #f])
...
;          [⊢ v ≫ v- ⇒ TY-expander]
  ...
  [(_ TY:id [A:id (~datum :) τA] ... (~datum :) ; params
   ...
   #:with (((xrec irec ...) ...) ...)
          (find-recur/i2 #'TY (stx-length #'(i ...)) #'(([i+x τin] ...) ...))
   ...
   #:with OUTPUT-DEFS
    #'(begin-
        ...
        (define-type C : [A*C : τA*C] ... [i+x : τin] ... -> τout) ...
        ...
        (define-typerule/red (elim-TY v P m ...) ≫
          ...
          ;; #:do[(displayln (stx->datum #'((CA ...) ...)))
          ;;      (displayln (stx->datum #'((τin ... τout) ...)))]
          ;; #:with ((τin/A ... τout/A) ...)
          ;;        (stx-map
          ;;         (λ (As τs) (substs #'(A ...) As τs))
          ;;         #'((CA ...) ...)
          ;;         #'((τin ... τout) ...))
          ;;        #:do[(displayln (stx->datum #'((τin/A ... τout/A) ...)))]
          ...
          #:with (((~literal TY) τoutA ... τouti ...) ...) #'(τout ...)
