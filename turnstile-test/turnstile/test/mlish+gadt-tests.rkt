...
(require turnstile/examples/gadt)
...
(check-type (I 10) : (Expr Int))
(check-type (B #f) : (Expr Bool))
(check-type (Add (I 10) (I 20)) : (Expr Int))
(check-type (Mul (I 30) (I 40)) : (Expr Int))
(check-type (Eq (I 10) (I 20)) : (Expr Bool))

(typecheck-fail (I #f) #:with-msg "couldn't unify Int and Bool")
(typecheck-fail (B 10) #:with-msg "couldn't unify Bool and Int")
(typecheck-fail (Add (B #f) (B #f)) #:with-msg "couldn't unify Int and Bool")
(typecheck-fail (Eq (B #f) (B #f)) #:with-msg "couldn't unify Int and Bool")
(typecheck-fail (Add (Eq (B #f) (B #f))
                     (Eq (B #f) (B #f)))
...
(check-type (eval (I 1)) : Int -> 1)
(check-type (eval (B #t)) : Bool -> #t)
(check-type (eval (Add (I 1) (I 2))) : Int -> 3)
(check-type (eval (Mul (I 3) (I 4))) : Int -> 12)
(check-type (eval (Eq (I 1) (I 1))) : Bool -> #t)
(check-type (eval (Eq (I 2) (I 3))) : Bool -> #f)
(check-type (eval (Eq (Add (I 1) (I 2)) (I 3))) : Bool -> #t)
(check-type (eval (Eq (Mul (Add (I 1) (I 2)) (I 4))
                      (Add (Mul (I 2) (I 3)) (I 6))))
            : Bool -> #t)

;; wont complain about missing clauses
;; bc all Int cases covered
(define (evalInt [e : (Expr Int)] -> Int)
  (match-gadt e with
    [I n       -> n] ; Int
    [Add e1 e2 -> (+ (eval e1) (eval e2))] ; Int
    [Mul e1 e2 -> (* (eval e1) (eval e2))]))

(check-type (evalInt (I 10)) : Int -> 10)
(check-type (evalInt (Add (I 1) (I 2))) : Int -> 3)
(check-type (evalInt (Mul (Add (I 1) (I 2)) (I 3))) : Int -> 9)
(typecheck-fail (evalInt (B #t)) #:with-msg "couldn't unify Int and Bool")
(typecheck-fail (evalInt (Eq (I 1) (I 2))) #:with-msg "couldn't unify Int and Bool")


;; wont complain about missing clauses
;; bc all Int cases covered
;; - TODO: extra cases ok? should this be err?
(define (evalInt/extra [e : (Expr Int)] -> Int)
  (match-gadt e with
    [I n       -> n] ; Int
    [B b       -> b] ; cant get here          
    [Add e1 e2 -> (+ (eval e1) (eval e2))] ; Int
...
(check-type (evalInt/extra (I 10)) : Int -> 10)
(check-type (evalInt/extra (Add (I 1) (I 2))) : Int -> 3)
(check-type (evalInt/extra (Mul (Add (I 1) (I 2)) (I 3))) : Int -> 9)
(typecheck-fail (evalInt/extra (B #t)) #:with-msg "couldn't unify Int and Bool")
(typecheck-fail (evalInt/extra (Eq (I 1) (I 2))) #:with-msg "couldn't unify Int and Bool")

;; wont complain about missing clauses
;; bc all Bool cases covered
(define (evalBool [e : (Expr Bool)] -> Bool)
  (match-gadt e with
    [B b       -> b]
    [Eq e1 e2 -> (= (eval e1) (eval e2))]))

(check-type (evalBool (B #t)) : Bool -> #t)
(check-type (evalBool (Eq (I 1) (I 2))) : Bool -> #f)
(typecheck-fail (evalBool (I 1))
                #:with-msg "couldn't unify Bool and Int")
(typecheck-fail (evalBool (Mul (Add (I 1) (I 2)) (I 3)))
                #:with-msg "couldn't unify Bool and Int")

;; exhaustiveness fail
(typecheck-fail
 (match-gadt (I 10) with
    [I n       -> n] ; Int
    [Add e1 e2 -> (+ (eval e1) (eval e2))])
 #:with-msg "match: clauses not exhaustive; missing: Mul")
(typecheck-fail
 (match-gadt (I 10) with
    [I n       -> n] ; Int
    [Mul e1 e2 -> (* (eval e1) (eval e2))])
 #:with-msg "match: clauses not exhaustive; missing: Add")

(typecheck-fail
 (match-gadt (B #t) with
    [B b       -> b])
 #:with-msg "match: clauses not exhaustive; missing: Eq")

(typecheck-fail
 (match-gadt (B #t) with
    [Eq e1 e2 -> (= (eval e1) (eval e2))])
 #:with-msg "match: clauses not exhaustive; missing: B")

;; remaining tests copied from mlish-tests.rkt ------------------------------
