...
;; NB: Provided only for testing
(provide
 ∀
 →
 (for-syntax covariant-Xs?))
    ...
    [(_ (Name:id X:id ...)
     ...
     #`(begin-
         (define-syntax NameExtraInfo
           (make-extra-info-transformer #'(X ...) #'(#%plain-app- (#%plain-app- 'c.Cons 'StructName Cons? [#%plain-app- acc c.τ] ...) ...)))
...
(define-typed-syntax in-range
  ...
  [(_ start end step)
   ...
   (⊢ (in-range- e- ...) : #,(mk-Sequence- (list Int+)))])
